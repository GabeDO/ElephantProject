...
NonAncestralSNP = str(snakemake.input[0])
AncestalSNP = str(snakemake.input[1])
    ...
    for line in line_reader:
        ...
        if line[0] != "#CHROM":
            if line[0].replace(".", "_") in locals (): # IF THE CHROMDICTONARY EXISTS
                ...
                eval(line[0].replace(".", "_"))[line[1]] += [line[3]]
    ...
    for line in line_reader:
        if line[0] == "CHROM":
            ...
            if line[0].replace(".", "_") in locals ():
                if line[1] in eval(line[0].replace(".", "_")):
                    Population_SNPs.append([line[0],line[1],line[2],line[3],str((int(line[5]))-int(line[4])),line[5],str((int(line[5]))-int(line[6])),str((int(line[5]))-int(line[7])),str((int(line[5]))-int(line[8])),"Polarized"])
                    print("ZZZZap!")
                else:
                    Population_SNPs.append([line[0],line[1],line[2],line[3],line[4],line[5],line[6],line[7],line[8],"Not_polarized"])
            else:
                pass
