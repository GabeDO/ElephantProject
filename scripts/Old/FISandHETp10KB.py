...
FisPer100kbDic = defaultdict(list)
ExpHetPer100kbDic = defaultdict(list)
        ...
        for row in rows:
            ...
            if row[0][0] == "#":
                if row[0] == "#CHROM":
                    for i in row[9:]:
                        ElephantID = i.split("/")
                        csv_columns.append(ElephantID[1])
                else:
                    pass
            elif "INDEL" in  row[7]:
                swag += 1
                ...
                FisList.append(Fis)
                PosBin = int(row[1]) - int(row[1])%100000
                FisKey = str(row[0])+","+str(PosBin)
                FisPer100kbDic[FisKey] += [Fis]
                ExpHetPer100kbDic[FisKey] += [HetExp]
...
#Output Fis per 10kb
FisOutputfile = str(snakemake.output[2])
Fiscsv_file = FisOutputfile 
with open(Fiscsv_file, "w", newline="") as csvfile:
    writer = csv.writer(csvfile)
    writer.writerow(["Chrom","POS","Fis","ExpHet"])

    for key, values in FisPer100kbDic.items():
        IDforRow = key.split(",")
        RowToWrite = [IDforRow[0]] + [int(IDforRow[1])] + [np.mean(values)] + [np.mean(ExpHetPer100kbDic[key])]
        writer.writerow(RowToWrite)
                    
                
# Num of Hets per 100,000 bp
